...
describe("The Chuck Norris Hubot Script (integration)", function () {
    ...
    it("should not respond at all if not called specifically", function (done) {
        var response = new FakeResponse();
        _this.robot.overhears("hello", response);
        ChuckNorris(_this.robot);
        setTimeout(function () {
            response.waitForMessageToBeSent().then(function () {
